...
INSERT INTO gpkg_geometry_columns VALUES('field_project','geometry','POLYGON',4326,0,0);

INSERT INTO gpkg_extensions VALUES('field_project','geometry','gpkg_rtree_index','http://www.geopackage.org/spec120/#extension_rtree','write-only');

INSERT INTO gpkg_contents VALUES('field_project','features','field_project','','2024-04-17T13:25:04.729Z',NULL,NULL,NULL,NULL,4326);
...
INSERT INTO sqlite_schema(type, name, tbl_name, rootpage, sql) VALUES('table','rtree_field_project_geometry','rtree_field_project_geometry',0,'CREATE VIRTUAL TABLE "rtree_field_project_geometry" USING rtree(id, minx, maxx, miny, maxy)');


CREATE TABLE IF NOT EXISTS "rtree_field_project_geometry_rowid"(rowid INTEGER PRIMARY KEY, nodeno);


CREATE TABLE IF NOT EXISTS "rtree_field_project_geometry_node"(nodeno INTEGER PRIMARY KEY, data);
...
CREATE TABLE IF NOT EXISTS "rtree_field_project_geometry_parent"(nodeno INTEGER PRIMARY KEY, parentnode);


CREATE TRIGGER "rtree_field_project_geometry_insert" AFTER INSERT ON "field_project" WHEN (new."geometry" NOT NULL AND NOT ST_IsEmpty(NEW."geometry")) BEGIN INSERT OR REPLACE INTO "rtree_field_project_geometry" VALUES (NEW."fid",ST_MinX(NEW."geometry"), ST_MaxX(NEW."geometry"),ST_MinY(NEW."geometry"), ST_MaxY(NEW."geometry")); END;


CREATE TRIGGER "rtree_field_project_geometry_update1" AFTER UPDATE OF "geometry" ON "field_project" WHEN OLD."fid" = NEW."fid" AND (NEW."geometry" NOTNULL AND NOT ST_IsEmpty(NEW."geometry")) BEGIN INSERT OR REPLACE INTO "rtree_field_project_geometry" VALUES (NEW."fid",ST_MinX(NEW."geometry"), ST_MaxX(NEW."geometry"),ST_MinY(NEW."geometry"), ST_MaxY(NEW."geometry")); END;


CREATE TRIGGER "rtree_field_project_geometry_update2" AFTER UPDATE OF "geometry" ON "field_project" WHEN OLD."fid" = NEW."fid" AND (NEW."geometry" ISNULL OR ST_IsEmpty(NEW."geometry")) BEGIN DELETE FROM "rtree_field_project_geometry" WHERE id = OLD."fid"; END;


CREATE TRIGGER "rtree_field_project_geometry_update3" AFTER UPDATE ON "field_project" WHEN OLD."fid" != NEW."fid" AND (NEW."geometry" NOTNULL AND NOT ST_IsEmpty(NEW."geometry")) BEGIN DELETE FROM "rtree_field_project_geometry" WHERE id = OLD."fid"; INSERT OR REPLACE INTO "rtree_field_project_geometry" VALUES (NEW."fid",ST_MinX(NEW."geometry"), ST_MaxX(NEW."geometry"),ST_MinY(NEW."geometry"), ST_MaxY(NEW."geometry")); END;


CREATE TRIGGER "rtree_field_project_geometry_update4" AFTER UPDATE ON "field_project" WHEN OLD."fid" != NEW."fid" AND (NEW."geometry" ISNULL OR ST_IsEmpty(NEW."geometry")) BEGIN DELETE FROM "rtree_field_project_geometry" WHERE id IN (OLD."fid", NEW."fid"); END;


CREATE TRIGGER "rtree_field_project_geometry_delete" AFTER DELETE ON "field_project" WHEN old."geometry" NOT NULL BEGIN DELETE FROM "rtree_field_project_geometry" WHERE id = OLD."fid"; END;
