...
class Crag(CragProject):
    ...
    def __init__(self, iface: QgisInterface):
        ...
        logger.debug("CRAG plugin initialised.")
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('Crag', message)
    ...
    def add_action(
        ...
        if add_to_toolbar:
            # Adds plugin icon to CRAG toolbar
    ...
    def initGui(self):
        ...
        self.quick_map_tool_buttons["crag_locality_point_add"] = self.add_action(
        ...
        self.quick_map_tool_buttons["crag_locality_point_edit"] = self.add_action(
        ...
        self.quick_map_tool_buttons["crag_locality_point_delete"] = self.add_action(
        ...
        self.quick_map_tool_buttons["crag_lines_add"] = self.add_action(
        ...
        self.quick_map_tool_buttons["crag_lines_edit"] = self.add_action(
        ...
        self.quick_map_tool_buttons["crag_lines_delete"] = self.add_action(
        ...
        for line_table in FEATURE_TABLES_LINES:
            self.quick_map_tool_buttons[f"crag_{line_table}_add"] = self.quick_map_tool_buttons["crag_lines_add"]
    ...
    def set_vector_layer_properties(self, vector_layers: list[QgsVectorLayer]) -> None:
        ...
        for vector_layer in vector_layers:
            ...
            if vector_layer.name() in layer_dependencies:
                dependent_layer = self.get_crag_layer(layer_dependencies[vector_layer.name()])
    ...
    def set_view_lithology_rules(self) -> None:
        ...
        """
        view_lithology_layer = self.get_crag_layer("view_lithology")
    ...
    def export_qml_styles(self) -> bool:
        """
        Export the QML styles for layers which belong to the CRAG project.
        These are saved into the current project's styles directory.
        Returns a boolean indicating success of the process.
        """
        if not self.validate_qgis_state(project_active=True, db_file_exists=True, crag_layers_exist=True):
            return False
        ...
        for table_name in TABLE_LIST:
            ...
            if self.check_layer_exists(table_name) and layer_style_path.exists():
                ...
                layer = self.get_crag_layer(table_name)
    ...
    def select_quick_line_layer_add(self) -> bool:
        ...
        """
        if not self.validate_qgis_state(project_active=True, db_file_exists=True, crag_layers_exist=True, field_project_exists=True) or self.warn_unsaved_locality_data():  # noqa
        ...
        self.quick_map_tool_buttons["crag_lines_add"].setChecked(True)
    ...
    def confirm_line_layer_selector(self, line_layer: str, line_type: str) -> None:
        """
        Confirm the selection from the line layer selector and toggle the required tool.
        """
    ...
    def toggle_quick_map_tool(
        ...
        if layer_name == "field_project":
            check_fp = False
        if not self.validate_qgis_state(project_active=True, db_file_exists=True, crag_layers_exist=True, field_project_exists=check_fp) or self.warn_unsaved_locality_data():  # noqa
            # Disable active tool if there is one and untoggle buttons to ensure things are not left in a bad state
            self.disable_current_quick_map_tool()
            self.untoggle_quick_map_tool_buttons()
            return False
        ...
        if isinstance(layer_name, str):
            toggled_quick_map_tool_name = f"crag_{layer_name}_{mode}"
            layer = self.get_crag_layer(layer_name)
        else:
            toggled_quick_map_tool_name = f"crag_{layers_ref}_{mode}"
            layer = [
                self.get_crag_layer(layer_name_)
    ...
    def disable_current_quick_map_tool(self) -> None:
        ...
        if map_tool is not None and map_tool.toolName().startswith("crag_"):
    ...
    def warn_unsaved_locality_data(self) -> bool:
        ...
        parent_layer = self.get_crag_layer(parent_layer_name)
