...
class CragProject:
    ...
    def validate_qgis_state(
        ...
        if field_project_exists and not self.check_field_project_exists():
            ...
            further_details = ("To create and draw a new field_project polygon, go to:\n\n"
    ...
    def messagebar_warning_with_further_details(self, message: str, further_details: str) -> None:
        """
        Display a warning on the messagebar with a button for further deatils.
        """
        message_bar = self.iface.messageBar().createMessage("CRAG", message)
        further_details_button = QPushButton(message_bar)
        further_details_button.setText("Further Details")
        further_details_button.pressed.connect(lambda: QMessageBox.information(None, "Information", further_details))
        message_bar.layout().addWidget(further_details_button)
